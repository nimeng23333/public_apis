...
const API_URL = "https://api.publicapis.org"; //这个网站失效了
...
router.get('/', async function(req, res, next) {
    try {
        ...
        //let categorizedAPI = categorizeData(result.data.entries);
        //res.render("index", { data: {categories: categories.data.categories, categorizedAPI : categorizedAPI}});网站失效了导致上面四行要改
        let categorizedAPI = categorizeData(entries.entries)
        console.log(categorizedAPI);
        res.render("index",{data: {categories: categories.categories, categorizedAPI:categorizedAPI}})
      } catch (error) {
        res.render("index", { APIs: JSON.stringify(error.response) });
      }
    // let categorizedAPI = categorizeData(entries.entries);
    // res.render("index", { data: {categories: categories.categories, categorizedAPI : categorizedAPI}});
});
